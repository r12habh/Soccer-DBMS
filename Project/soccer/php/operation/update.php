...
if($inputchoice=="Player"){
	$sqlP = "Select playerid from football_league.player order by playerid ASC";
	$resultP = $conn-> query($sqlP);
	?>
	<div class="s003">
	<form action="update/pupdate.php" method="post">
		<select data-trigger="" name="choices-single-defaul" style="padding:20px; font-size: 160%;">
            ...
            while($row = $resultP->fetch_assoc())
            {
             echo "<option value='".$row[playerid]."'>".$row[playerid]."</option>";
			  ...
			  <button onClick="update/pupdate.php" style="margin:20px; padding:20px; font-size: 160%;">Submit</button>
			 </form>	
	</div>
	<?php
}
	...
	?>
	<div class="s003">
	<form action="update/supdate.php" method="post">
		<select data-trigger="" name="choices-single-defaul" style="padding:20px; font-size: 160%;">
                <option placeholder="">Player ID</option>
				<?php
				if($resultS){
            while($row = $resultS->fetch_assoc())
            {
             echo "<option value='".$row[playerid]."'>".$row[playerid]."</option>";
			  ...
			  <button onClick="update/supdate.php" style="margin:20px; padding:20px; font-size: 160%;">Submit</button>
			 </form>	
	</div>
	<?php
}
	...
	?>
	<div class="s003">
	<form action="update/cupdate.php" method="post">
		<select data-trigger="" name="choices-single-defaul" style="padding:20px; font-size: 160%;">
            ...
            while($row = $resultC->fetch_assoc())
            {
             echo "<option value='".$row[coachid]."'>".$row[coachid]."</option>";
			  ...
			  <button onClick="update/cupdate.php" style="margin:20px; padding:20px; font-size: 160%;">Submit</button>
			 </form>
</div>			 
